    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                ...
                if labels or conf[j] > 0.25:  # 0.1 conf thresh
...
def plot_study_txt(f='study.txt', x=None):  # from utils.general import *; plot_study_txt()
    ...
    for f in ['study/study_dota_yolo%s.txt' % x for x in ['s', 'm', 'l', 'x']]:
        ...
        ax2.plot(y[6, :j], y[3, :j] * 1E2, '.-', linewidth=2, markersize=8,
                 label=Path(f).stem.replace('study_dota_', '').replace('yolo', 'YOLO'))
